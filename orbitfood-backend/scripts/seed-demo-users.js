...
async function ensureAdminUser() {
    ...
    const user = await db.User.create({
        ...
        address: '5th Floor OrbitFood, WTT',
...
async function ensureTenantUser() {
    const existing = await db.User.findOne({ where: { email: 'manager@orbitfood.test' }, disableTenantCheck: true });
    if (existing) {
