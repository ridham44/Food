# OrbitFood Backend (orbitfood-backend)
...
- **`scripts/seed-demo-users.js`** (`node scripts/seed-demo-users.js`) — idempotent dev convenience script that ensures one demo login exists per role: Admin (`john@example.com` / `admin@123`), Tenant (`manager@orbitfood.test` / `manager@123`), and Customer (phone `9998877766`, OTP `1234`). Not wired into the seeder/migration pipeline — run it manually after `db:seed:all` when you need working logins for local testing.
