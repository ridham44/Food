...
type AlicaVariant = 'default' | 'mobile-tab-bar' | 'inline';
...
interface AlicaWidgetProps {
  ...
  greeting: string;
  suggestions: string[];
  /** 'inline' embeds Alica as a static panel in the page layout instead of a floating FAB — used for the "Meet Alicia" dashboard card. */
  variant?: AlicaVariant;
  /** Decorative background image for the panel (behind a dark overlay so text stays legible) — only meaningful with variant="inline". */
...
const FAB_OFFSET: Record<'default' | 'mobile-tab-bar', string> = {
  default: 'bottom-6',
  // The customer shell's bottom tab bar only shows below the `lg` sidebar
  // breakpoint (see CustomerLayout) — match it here so the FAB doesn't
  // overlap the tab bar between sm and lg.
  'mobile-tab-bar': 'bottom-24 lg:bottom-6',
};

const PANEL_OFFSET: Record<'default' | 'mobile-tab-bar', string> = {
  default: 'bottom-24',
  'mobile-tab-bar': 'bottom-40 lg:bottom-24',
};

let idCounter = 0;
const nextId = () => `alica-${++idCounter}`;

export function AlicaWidget({ apiClient, endpoint, greeting, suggestions, variant = 'default', backgroundImage, onAsk }: AlicaWidgetProps) {
  const isInline = variant === 'inline';
  const [open, setOpen] = useState(false);
  const panelVisible = isInline || open;
    ...
    <>
      {!isInline && (
        <button
          ...
          className={cn(
            ...
            FAB_OFFSET[variant as 'default' | 'mobile-tab-bar']
          )}
        >
      ...
      {panelVisible && (
        <div
          ...
          aria-modal={isInline ? undefined : false}
          aria-label={isInline ? undefined : 'Alica, your AI assistant'}
          className={
            isInline
              ? 'glass-panel--strong relative flex h-full min-h-[24rem] flex-col overflow-hidden rounded-card'
              : cn(
                  ...
                  PANEL_OFFSET[variant as 'default' | 'mobile-tab-bar']
                )
          }
        >
          {isInline && backgroundImage && (
            <>
              <div
                className="pointer-events-none absolute inset-0 bg-cover bg-top opacity-25"
                style={{ backgroundImage: `url(${backgroundImage})` }}
                aria-hidden="true"
              />
              <div
                className="pointer-events-none absolute inset-0 bg-gradient-to-b from-bg-deep/40 via-bg-deep/85 to-bg-deep"
                aria-hidden="true"
              />
            </>
          )}

          <div className={cn('flex h-full flex-col', isInline && backgroundImage && 'relative z-10')}>
            <div className="flex items-center gap-3 border-b border-border-subtle px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-semibold text-text-primary">{isInline ? 'Meet Alicia' : 'Alica'}</p>
                <p className="truncate text-xs text-text-muted">{isInline ? 'Your Food Assistant' : 'Your AI assistant'}</p>
              </div>
              {!isInline && (
                <button
                  type="button"
