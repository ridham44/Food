...
const generateSortable = (query, model, columnOptions) => {
    ...
    let mainSort = query.sort ? JSON.parse(query.sort) : [];

    // fields to be sorted when group is used. Default [].
    let groupSort = query?.group ? JSON.parse(query?.group) : [];
...
const formatField = (fieldName, operator, fieldValue, columnOptions, model) => {
    ...
    if (columnOpt && columnOpt.isCustom) {
        ...
        obj[fieldKey] = {
            [sequelizeOperators[operator].key]: sequelizeOperators[operator].starts + `${fieldValue}` + sequelizeOperators[operator].ends,
        };
    }
 ...
 */
const devExtremeFilters = async (query, model, includes, columnOptions = {}) => {
    ...
    // ** search filters
    if (query?.filter) {
        let queryFilters = query.filter ? JSON.parse(query.filter) : [];
